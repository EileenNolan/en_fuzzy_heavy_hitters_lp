...
//use crate::gadgets::BinaryGadgets; // or wherever the extract trait is defined
    ...
    F::Item: Clone,
{
    ...
    //Ok(BinaryBundle::from(sliced))
    //Ok(BinaryBundle::from(Bundle::from(sliced)))
    //Ok(BinaryBundle::from(Bundle(sliced)))
    ...
    C: AbstractChannel + std::clone::Clone,
{
    ...
    let is_negative = fancy_sum_is_negative::<Garbler<C, AesRng, OtSender, AllWire>>(&mut gb, circuit_wires).unwrap();
    ...
    C: AbstractChannel + std::clone::Clone,
{
    ...
    let is_negative = fancy_sum_is_negative::<Evaluator<C, AesRng, OtReceiver, AllWire>>(&mut ev, circuit_wires).unwrap();
...
where
    F: FancyReveal + Fancy + BinaryGadgets + FancyBinary + FancyArithmetic,
    F::Item: Clone, // Needed for extract()
{
    // Add the two secret inputs
    let sum = f.bin_addition_no_carry(&wire_inputs.garbler_wires, &wire_inputs.evaluator_wires)?;

    // Get the MSB (most significant bit), which is 1 if the sum is negative in 2's complement
    //let msb = f.extract(&sum, 127, 1)?;
    //let msb = extract(&sum, 127, 1)?;
...
fn main() {
    ...
    // Compute expected MSB (bit 127) of the true sum
    let sum = gb_value.wrapping_add(ev_value); // Prevent overflow panic
    ...
    assert!(
        result == expected_msb,
        "The garbled circuit result is incorrect. Expected MSB = {expected_msb}"
    );
}
